...
IMG_DATA_DIR = os.environ.get(
    "RIVERTHON_IMG_DATA_DIR",
    os.path.join(BASE_DIR, "../data/Satellite data/IMG_DATA"),
)
...
def make_rgb_image(b2_path, b3_path, b4_path, out_path):
    ...
    print(f"Saved before flood image: {out_path}")


def generate_all_before_images():
    ...
    for resolution_dir in resolution_dirs:
        ...
        out_path = os.path.join(OUTPUT_DIR, f"before_flood_{resolution_name}.jpg")
        make_rgb_image(b2_path, b3_path, b4_path, out_path)


if __name__ == "__main__":
    generate_all_before_images()
